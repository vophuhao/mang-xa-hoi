...
export default function FollowerModal({ isOpen, onClose, followersData = [], followingData = [] }) {
    ...
    // Theo dõi hoặc bỏ theo dõi lại
    const handleFollowBack = async (targetUserId, isFollowingBack) => {
        const prevIsFollowingBack = isFollowingBack;
        ...
        try {
            ...
            setLocalFollowBackIds((prev) => {
                const updated = new Set(prev);
                if (prevIsFollowingBack) updated.add(targetUserId);
                ...
                <div className="max-h-[350px] overflow-y-auto divide-y divide-gray-100 dark:divide-gray-800">
                    {filteredFollowers.length > 0 ? (
                                    ...
                                    {!isCurrentUser && (
                                        ...
                                        >
                                            {isPending ? (
                                                ...
                                                <>{isFollowingBack ? "Đang theo dõi" : "Theo dõi"}</>
                                            )}
                                        </button>
                                    )}
                                </div>
                        ...
                        <div className="flex flex-col items-center justify-center text-center gap-1 py-10 text-gray-500 dark:text-gray-400">
                            <Search size={20} />
                            <p className="text-sm">Không có người theo dõi nào</p>
                        </div>
                    )}
                </div>
            </div>
