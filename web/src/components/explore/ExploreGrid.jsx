...
const ExploreGrid = ({ searchQuery, onPostClick }) => {
  ...
  return (
    <>
      {/* Desktop Grid (3 columns, masonry-like) */}
      <div className="hidden lg:grid lg:grid-cols-3 lg:gap-1">
        {filteredPosts.map((post, index) => (
          <ExploreGridItem
            key={post._id}
            post={post}
            onClick={() => onPostClick(post)}
            className={`${getGridItemClass(index)} cursor-pointer`}
          />
        ))}
      </div>

      {/* Tablet Grid (2 columns) */}
      <div className="hidden md:grid md:grid-cols-2 md:gap-1 lg:hidden">
