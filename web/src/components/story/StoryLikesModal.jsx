...
const StoryLikesModal = ({ isOpen, onClose, storyId, currentStory, isOwnStory = false }) => {
  const { data: likes } = useStoryLikes(storyId, isOpen);
  const { data: viewers, isLoading: viewersLoading } = useStoryViewers(storyId, isOwnStory);
  ...
  return (
    ...
    >
      {/* Modal */}
      <div className="animate-in fade-in-0 zoom-in-95 max-h-[80vh] w-full max-w-md rounded-xl bg-white shadow-2xl duration-300 dark:bg-gray-800">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-gray-200 p-4 dark:border-gray-700">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Chi tiết Story</h2>
          <button
            onClick={onClose}
        ...
        <div className="max-h-96 overflow-y-auto">
          ...
          <div className="border-b border-gray-200 p-4 dark:border-gray-700">
            <div className="flex items-center justify-center text-center">
              <div className="flex flex-col items-center">
                ...
                <span className="text-sm text-gray-500 dark:text-gray-400">Lượt xem</span>
              </div>
              {/* Only show likes count if there are likes */}
              {currentStory?.likeCount > 0 && (
                <>
                  <div className="mx-6 h-8 w-px bg-gray-300 dark:bg-gray-600"></div>
                  <div className="flex flex-col items-center">
                    <div className="flex items-center space-x-1 text-2xl font-bold text-gray-900 dark:text-white">
                      <Heart size={20} className="text-red-500" />
                      <span>{currentStory?.likeCount}</span>
                    </div>
                    <span className="text-sm text-gray-500 dark:text-gray-400">Lượt thích</span>
                  </div>
                </>
              )}
            </div>
          ...
          <div className="p-4">
            <h3 className="mb-3 flex items-center text-sm font-semibold text-gray-900 dark:text-white">
              ...
              Người đã xem ({viewers?.viewers?.length || 0})
            </h3>
